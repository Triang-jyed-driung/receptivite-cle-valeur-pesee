...
__all__ = ["AdaMin", "AdaMin"]



class AdaMin(Optimizer):
    def __init__(
        self,
        params: ParamsT,
    ...
    def _init_group(
        ...
        for p in group["params"]:
            ...
            if p.grad.is_sparse:
                raise RuntimeError("AdaMin does not support sparse gradients")
            grads.append(p.grad)
...
AdaMin.__doc__ = (
    ...
    Args:
        ...
        {_fused_doc}
    .. Note::
        A prototype implementation of Adam and AdaMin for MPS supports `torch.float32` and `torch.float16`.
...
def _single_tensor_AdaMin(
    ...
    *,
    ...
    for i, param in enumerate(params):
        ...
        # gsm = grad.square_().mean()
        exp_avg_sq.lerp_(grad.square_().mean(), 1-beta2)
        # exp_avg_sq = torch.maximum(grad.square_().mean(), beta2 * exp_avg_sq)
        # exp_avg_sq = (torch.lerp(exp_avg_sq.square(), gsm.square(), 1-beta2**2)).sqrt()

        if step_t % 50 == 2:
            try: wandb.log({f"expavgsq_{i}_{grad.nelement()}": float(exp_avg_sq)}, commit=False)
            except: pass
        ...
        if capturable or differentiable:
            ...
            if step_t % 50 == 2:
                try: wandb.log({f"update_{i}_{grad.nelement()}": step_size / float(denom)}, commit=False)
                except: pass
...
@_disable_dynamo_if_unsupported(single_tensor_fn=_single_tensor_AdaMin)
def AdaMinFunc(
    params: List[Tensor],
    grads: List[Tensor],
    exp_avgs: List[Tensor],
    ...
    *,
    ...
    r"""Functional API that performs AdaMin algorithm computation.

    See :class:`~torch.optim.AdaMin` for details.
    """
    ...
    if fused and not torch.jit.is_scripting():
        ...
        func = _multi_tensor_AdaMin
    else:
        func = _single_tensor_AdaMin
